...
import { getMapCamList } from '@/api/defaultApi/index';
...
export default class mapCamInfo {
   ...
   */
                ...
                switch (type) {
                    ...
                    // 创建点
                        this.getMapPointData(
                            data, code, bubble, model, type, el, cb
                        );
                        break;
                    case 'SURFACE':
                        // 创建面
                        this.getMapPolygonData(
                            data, code, type, bubble, el, cb
                        );
                        break;
                    case 'LINE':
                        this.getMapPolygonData(
    ...
    // 创建地图点位类型为 点
    getMapPointData(
        data:any, code:any, bubble:any, model:any, type:string, el?:MapMarkerParam, cb?:Function
    ) {
    ...
    getMapPolygonData(
        data:any, code:any, type:string, bubble:any, el?:MapMarkerParam, cb?:Function
    ) {
        data.forEach((element:any) => {
            if (element[code].cb_data) {
                const cbData = JSON.parse(element[code].cb_data);
    ...
    createMapBubble(
        ...
        if (el?.url && el?.url === '') console.error('请传入对应模型地址或者图片地址');
