...
import { getScene, getSceneList } from '@/baseTi/api';
...
export default class CampusController extends SceneBase {
    ...
    campusLoad(event: any) {
        ...
        that.app.level.options.autoBakeFloor = false;
        // 开启默认的层级控制
        ...
        if (!initChildScene) {
            ...
            for (let i = 0; i < building.length; i += 1) {
                const index = this.campusMap.children.findIndex((item: Aobject) => String(item.campusBuilderId) === String(building[i].uuid));
                if (
                    index > 0 &&
        ...
        that.app.on(
            ...
            (ev: Aobject) => {
                ...
                if (current.type === 'Campus') {
                    ...
                    if (bd.floors) {
                        const index = this.campusMap.children.findIndex((item: Aobject) => String(item.campusBuilderId) === String(bd.uuid));
